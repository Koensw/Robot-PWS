...
static PWMConfig motor_pwmcfg = {10000, /* 10Khz PWM clock frequency */
...
int motorActive(void) { return (motorLeftActive() && motorRightActive()); }
...
void motor_left_on(Direction dir) {
  ...
  if (dir == D_FORWARD) {
    pwmEnableChannelI(motor_pwm, lp1, PWM_PERCENTAGE_TO_WIDTH(motor_pwm, 0));
    pwmEnableChannelI(motor_pwm, lp2,
                      PWM_PERCENTAGE_TO_WIDTH(motor_pwm, motor_speed));
  } else {
    pwmEnableChannelI(motor_pwm, lp1,
                      PWM_PERCENTAGE_TO_WIDTH(motor_pwm, motor_speed));
    pwmEnableChannelI(motor_pwm, lp2, PWM_PERCENTAGE_TO_WIDTH(motor_pwm, 0));
  }
  chSysUnlock();
...
void motor_right_on(Direction dir) {
  ...
  if (dir == D_FORWARD) {
    pwmEnableChannelI(motor_pwm, rp1, PWM_PERCENTAGE_TO_WIDTH(motor_pwm, 0));
    pwmEnableChannelI(motor_pwm, rp2,
                      PWM_PERCENTAGE_TO_WIDTH(motor_pwm, motor_speed));
  } else {
    pwmEnableChannelI(motor_pwm, rp1,
                      PWM_PERCENTAGE_TO_WIDTH(motor_pwm, motor_speed));
    pwmEnableChannelI(motor_pwm, rp2, PWM_PERCENTAGE_TO_WIDTH(motor_pwm, 0));
  }
  chSysUnlock();
