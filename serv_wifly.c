...
char char_mem[WIFLY_MEM_SIZE][WIFLY_EL_SIZE] __attribute__((aligned(sizeof(stkalign_t))));;

Thread *wifly_tp;
static WORKING_AREA(wifly_thread_area, 4096);

//initalizes wifly
int wiflyInit(SerialDriver *serial, ioportid_t gpio, int tx, int rx, int baud, ProtocolCallback func){
	wifly_driver = serial;
	wifly_config.sc_speed = baud;
	wifly_rec_func = func;
	
	sdStart(wifly_driver, &wifly_config);
	palSetPadMode(GPIOD, GPIOD_LED6, PAL_MODE_OUTPUT_PUSHPULL); // (FIXME: DEBUGGING)
...
State wiflySend(char *msg){
	if(strlen(msg) == 0) return S_OVERFLOW; //not correct length
	//ask thread to send message (if current thread is not executing this)
	if(chThdSelf() == wifly_tp) wifly_send(msg);
	else chMsgSend(wifly_tp,(msg_t) msg);
	return S_OK;
}
...
State wiflySendCommandOK(char *cmd){
	return wiflySendCommand(cmd, "AOK");
}
...
State wiflyRead(char *msg){
	...
	if(chThdSelf() == wifly_tp) return wifly_receive(msg, WIFLY_EL_SIZE);
	else return chMsgSend(wifly_tp,(msg_t) msg);
}
...
void wifly_send(char *msg){
    ...
    while (TRUE) {
		...
		if(flags & CHN_OUTPUT_EMPTY) break;
    }
}
...
State wifly_receive(char *msg, int max_l){
	...
	while(TRUE){
		...
		if(in == Q_TIMEOUT || in == Q_RESET) break;
...
msg_t wifly_loop(void *arg) {
	...
	chEvtRegisterMask((EventSource *)chnGetEventSource(wifly_driver), &wifly_el, 1);
	...
	while (TRUE) {
		...
		//wait for event
		if(chEvtWaitOneTimeout(1,TIME_IMMEDIATE) == 0) continue;
		...
		if(flags & CHN_INPUT_AVAILABLE) {
			...
			if(i == 16 || j == 1024){
				...
				if(i > 2 && id[0] == '*' && id[i-1] == '*'){
					//state
					if(strcmp(id, "*HELLO*") == 0) wifly_send("*HELLO*\r\n");
					else wifly_send("*ERROR*\r\n");
				}else{
					//message
					Message ret = wifly_rec_func(id, args);
					if(ret == M_OK) wifly_send("*OK*\r\n");
					else if(ret != M_MESSAGE) wifly_send("*ERROR*\r\n");
				}
