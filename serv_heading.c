...
int magSetRegisters(uint8_t reg1, uint8_t reg2, uint8_t reg3){
	...
	status = i2cMasterTransmitTimeout(driver, mag_addres, txbuf, 6, rxbuf, 0, tmo);
...
int magGetRegValues(){
	...
	txbuf[0]= 3;
    i2cAcquireBus(driver);
    status = i2cMasterTransmitTimeout(driver, mag_addres, txbuf, 1, rxbuf, 6, tmo);
    i2cReleaseBus(driver);
	if(status !=RDY_OK){
	}
...
float heading_calculate(){
	...
	float Yh = X_m * sinRoll * sinPitch + Y_m * cosRoll - Z_m * sinRoll * cosPitch;
